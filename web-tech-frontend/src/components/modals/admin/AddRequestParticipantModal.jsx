...
const AddRequestParticipantModal = ({
  isOpen,
  onClose,
  requestId,
  bloodType,
  ...
  try {
    const response = await api.get(`/requests/${requestId}/participants`);
    console.log("REQUEST PARTICIPANTS RESPONSE:", response.data);
  ...
  useEffect(() => {
    fetchDonors();
    fetchParticipants();
  }, [isOpen, search, bloodType, requestId]);

  const handleAdd = async (donorId) => {
    try {
      setAddingId(donorId);

      await api.post(`/requests/${requestId}/participants/manual`, {
        donorId,
      });
      ...
      console.error("Failed to add donor to request:", error);
        ...
        <div className="flex justify-between items-start p-6 border-b border-gray-100">
          <div>
            ...
            <p className="text-sm text-gray-500">
              Search existing donors with blood type <strong>{bloodType}</strong>
            </p>
          </div>
...
export default AddRequestParticipantModal;